...
with open(cmake_src_path) as f:
    for line in f:
        if "set(OpenBLAS_MAJOR_VERSION" in line:
            ver_major = extract_value(line)
            ver_major_get = True
        elif "set(OpenBLAS_MINOR_VERSION" in line:
            ver_minor = extract_value(line)
            ver_minor_get = True
        elif "set(OpenBLAS_PATCH_VERSION" in line:
...
for line in fileinput.input(utilfile, inplace = 1):
    if "set(VERSION_MAJOR " in line:
        print "    set(VERSION_MAJOR " + ver_major + ")"
    elif "set(VERSION_MINOR " in line:
            print "    set(VERSION_MINOR " + ver_minor + ")"
    elif "set(VERSION_PATCH " in line:
            print "    set(VERSION_PATCH " + ver_patch + ")"
    else:
        print line,
